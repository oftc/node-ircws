...
function validateReverseDns(client, next) {
  var dnsType = client.remoteParsedAddress.kind() === 'ipv6' ? 'AAAA' : 'A';

  dns.reverse(client.remoteParsedAddressString, function reverseCb(err, reverseResults) {
    if (err) {
    ...
    dns.resolve(reverseResults[0], dnsType, function forwardCb(err, forwardResults) {
      ...
      var parsedResults = forwardResults.map(ipaddr.parse).filter(function matchFn(item, index, array) {
        return client.remoteParsedAddress.match(item, client.remoteParsedAddress.kind() === 'ipv6' ? 128 : 32);
      });
...
DNSFilter.prototype._transform = function dnsfltrTransform(client, enc, cb) {
  ...
  vasync.parallel({
    ...
    if (reverse)
      ...
      client.hostname = client.remoteParsedAddressString;
